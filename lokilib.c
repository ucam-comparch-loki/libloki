...
inline void request_work() {
  asm (
    "syscall 17\n"              // Get core ID
    "or r0, r0, r0\n"           // no-op since result of syscall can't be forwarded
    "addu r0, r11, r0 -> 2\n"   // Send core ID to master
    : /* no outputs */
    : /* no inputs */
    : "r11"
  );
...
void worker_thread(const loop_config* config, const int worker) {
  ...
  asm (
    "syscall 17\n"           // Put core ID in r11
    "addui %0, r11, 2\n"     // Core 1 (first worker) connects to input 3 (second channel)
    : "=r" (channel)
    :
    : "r11"
  );
  int address = loki_core_address(0, 0, channel);
  SET_CHANNEL_MAP(2, address);
